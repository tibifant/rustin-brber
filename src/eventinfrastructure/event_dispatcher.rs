...
impl EventDispatcher {
    pub fn new(game_service_rest_adapter: Arc<dyn GameServiceRestAdapterTrait>) -> Self {
        Self {
            ...
            game_status_event_handler: Arc::new(GameStatusEventHandler::new(game_service_rest_adapter.clone())),
            round_status_event_handler: Arc::new(RoundStatusEventHandler::new(game_service_rest_adapter.clone())),
    ...
    pub async fn dispatch(&self, event: GameEvent) {
        match event.event_body {
            GameEventBodyType::GameStatus(game_status_event) => {
                self.game_status_event_handler.handle(game_status_event).await;
            }
            GameEventBodyType::RoundStatus(round_status_event) => {
                self.round_status_event_handler.handle(round_status_event).await;
            }
