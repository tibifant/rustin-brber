...
impl GameServiceRestAdapterTrait for GameServiceRestAdapterImpl {
    ...
    async fn get_all_games(&self) -> Result<Vec<GameInfoResponseBody>, Box<dyn Error>> {
        ...
        let response = self.client.async_client.get(&url).send().await.map_err(
            GameServiceRestAdapterImpl::handle_reqwest_error
        )?;

        if response.status() != StatusCode::OK {
            let status = response.status();
            let text = response.text().await.unwrap_or_else(|_| "".to_string());
            return Err(Box::new(GameServiceError::UnexpectedError(format!("Unexpected status code: {} \n Error Message: {}", status, text))));
        }
    ...
    async fn create_game(&self, max_players: u16, rounds: u16) -> Result<GameInfoResponseBody, Box<dyn Error>> {
        let url = format!("{}/games", self.game_host);
        ...
        let response = self.client.async_client.post(&url).json(&body).send().await.map_err(GameServiceRestAdapterImpl::handle_reqwest_error)?;
    ...
    async fn join_game(&self, game_id: &str) -> Result<bool, Box<dyn Error>> {
        ...
        let response = self.client.async_client.put(&url).send().await.map_err(GameServiceRestAdapterImpl::handle_reqwest_error)?;
    ...
    async fn send_command(&self, command: Command) -> Result<CommandInfoResponse, Box<dyn Error>> {
        ...
        let response = self.client.async_client.post(&url).json(&command).send().await.map_err(GameServiceRestAdapterImpl::handle_reqwest_error)?;
        match response.status() {
            StatusCode::CREATED => {
            ...
            StatusCode::BAD_REQUEST => {
                Err(Box::new(CommandError::MultipleCauseError(response.text().await.unwrap())))
            }
            StatusCode::NOT_FOUND => {
                Err(Box::new(CommandError::PlayerOrGameNotFound(response.text().await.unwrap())))
            }
            code => {
                Err(Box::new(CommandError::UnknownError(format!("Unexpected status code: {} \n {}", code, response.text().await.unwrap()))))
            }
        }
    }
    ...
    async fn register_player(&self) -> Result<Player, Box<dyn Error>> {
        ...
        let player = match response.status() {
            ...
            _ => {
                return Err(Box::new(PlayerError::UnexpectedError(format!("Unexpected status code: {}", response.status()))));
            }
        };
        Ok(player)
    }
    ...
    async fn fetch_player(&self) -> Result<Player, Box<dyn Error>> {
        ...
        let response = self.client.async_client.get(&url).query(&query).send().await.map_err(GameServiceRestAdapterImpl::handle_reqwest_error)?;
        match response.status() {
            StatusCode::OK => {
    ...
    async fn start_game(&self, game_id: &str) -> Result<(), Box<dyn Error>> {
        ...
        let response = self.client.async_client.post(&url).send().await.map_err(GameServiceRestAdapterImpl::handle_reqwest_error)?;
        match response.status() {
            StatusCode::CREATED => {
                info!("Started Game {} successfully! {:?}",game_id, response.text().await.unwrap());
            }
            StatusCode::BAD_REQUEST => {
            ...
            StatusCode::NOT_FOUND => {
                error!("Game {} could not be found: {:?}",game_id, response.text().await.unwrap());
            }
            _ => {
                error!("Unknown error occured when trying to start a game! {:?}", response.text().await.unwrap());
            }
        }
        Ok(())
    }
    ...
    async fn end_all_existing_games(&self) -> Result<(), Box<dyn Error>> {
        let games = self.get_all_games().await.map_err(|e| error!("Couldn't end existing games {}", e)).unwrap();
        for game in games {
            ...
            if game.game_status == GameStatus::CREATED {
                ...
                self.start_game(&game.game_id).await.map_err(|e| error!("Couldn't start game {}", e)).unwrap();
            ...
            match response.status() {
                ...
                StatusCode::NOT_FOUND => {
                    error!("Game {} can't be ended because it could not be found: {:?}",game.game_id, game);
                }
                _ => {
